...
int F1(int key);
int F2(int key);
bool search_linear(node* hashtable[], int key);
bool search_quad(node* hashtable[], int key);
void insertHash_linear(node* hashtable[], int key);
void insertHash_quad(node* hashtable[], int key);
void insertHash_linked(node* hashtable[], int key);
...
void insertHash_linear(node* hashtable[], int key) {
	if (search_linear(hashtable, key))return;
	int index = F1(key);
	while (hashtable[index] != NULL) {
		index = (index + 1) % M;
	}
...
void insertHash_quad(node* hashtable[], int key) {
	if (search_quad(hashtable, key) || N >= M - 1)return;
	int index = F1(key);
	int i = 1;
	while (hashtable[index] != NULL) {
		index = (F1(key) + i * i) % M;
...
bool exec(node* hashtable[], int arr[], int n, int mode) {
	...
	case 1:
		...
		init(hashtable);
		for (int i = 0; i < n; i++) {
			insertHash_linear(hashtable, arr[i]);
		}
		print(hashtable);
		break;
	case 2:
		...
		init(hashtable);
		for (int i = 0; i < n; i++) {
			insertHash_quad(hashtable, arr[i]);
		}
		print(hashtable);
		break;
	case 3:
		...
		init(hashtable);
		for (int i = 0; i < n; i++) {
			insertHash_linked(hashtable, arr[i]);
		}
		print(hashtable);
		break;
	case 4:
